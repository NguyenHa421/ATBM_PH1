...
BEGIN
  -- Dynamically construct password based on student ID (assuming unique)
    ...
    ELSIF v_oldrole = 'Giao vu' THEN
        EXECUTE IMMEDIATE 'REVOKE SELECT ON ADMIN.UV_NVXEMTHONGTIN FROM ' || v_user;
        EXECUTE IMMEDIATE 'REVOKE EXECUTE ON ADMIN.USP_CHINHSODT FROM ' || v_user;
        EXECUTE IMMEDIATE 'REVOKE SELECT, INSERT, UPDATE ON ADMIN.TB_SINHVIEN FROM ' || v_user;
        EXECUTE IMMEDIATE 'REVOKE SELECT, INSERT, UPDATE ON ADMIN.TB_DONVI FROM ' || v_user;
        EXECUTE IMMEDIATE 'REVOKE SELECT, INSERT, UPDATE ON ADMIN.TB_HOCPHAN FROM ' || v_user;
        EXECUTE IMMEDIATE 'REVOKE SELECT, INSERT, UPDATE ON ADMIN.TB_KHMO FROM ' || v_user;
        EXECUTE IMMEDIATE 'REVOKE SELECT, UPDATE ON ADMIN.TB_PHANCONG FROM ' || v_user;
        EXECUTE IMMEDIATE 'REVOKE SELECT, INSERT, DELETE ON ADMIN.TB_DANGKY FROM ' || v_user;
